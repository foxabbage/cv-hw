...
def main(args, extras) -> None:
    ...
    # ====== SWANLAB INTEGRATION START ======
    # 导入 SwanLab Logger
    from swanlab.integration.pytorch_lightning import SwanLabLogger
    # ====== SWANLAB INTEGRATION END ======
    ...
    if args.train:
        ...
        swanlab_logger = SwanLabLogger(
            project="3D_Vision_Challenge_ImageTo3D",
            experiment_name=f"ObjectB_{prompt_name}_{stage}",
            config=omegaconf.OmegaConf.to_container(cfg, resolve=True),
            logdir=os.path.join(cfg.trial_dir, "swanlab_logs")
        )
        # ====== SWANLAB INTEGRATION END ======
        
        loggers += [
            # 3. 只保留 SwanLab Logger（移除 TensorBoard/CSV）
            swanlab_logger,
        ] + system.get_loggers()
        rank_zero_only(
